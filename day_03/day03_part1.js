...
const manhatten = c => Math.abs(c[0]) + Math.abs(c[1]);
...
let keys = {};
// fill keys with 1
wire1Coordinates.forEach(c => keys[keyOf(c)] = 1);

// search for 2
let crossings = [];
wire2Coordinates.forEach((c) => {if (keys[keyOf(c)] == 1) {crossings.push(c)}});
console.log('Crossings');
console.log(crossings);

console.log('Manhatten distanes');
console.log(crossings.map((c) => {return [manhatten(c), c]}).sort((a,b) => a[0]-b[0]));
