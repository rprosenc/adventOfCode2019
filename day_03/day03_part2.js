...
const allCoordinatesForWire = function(wire) {
	let position = [0,0];
	return wire.split(',').map((step) => {
		const coordinates = allCoordinatesForStep(step, position);
...
const allCoordinatesForStep = function(step, position) {
	const direction = step[0];
	let count = step.substr(1);
...
const signalLength = c => c[1]+c[2];


const wire1Coordinates = allCoordinatesForWire(wire1);
const wire2Coordinates = allCoordinatesForWire(wire2);

console.log('Lengths');
console.log(wire1Coordinates.length);
console.log(wire2Coordinates.length);

let keys = {};
// fill keys with path lengths
wire1Coordinates.forEach((c,i) => {keys[c] = i+1});

// search crossings, save path lengths of wire1 and wire2
let crossings = [];
wire2Coordinates.forEach(
	(c) => {
		if (keys[c]  > 0 ) {
			crossings.push(
				[c, keys[c], wire2Coordinates.indexOf(c)+1 ]
			);
		}
	}
);
...
console.log('Manhatten distanes');
console.log(crossings.map((c) => {return [signalLength(c), c]}).sort((a,b) => a[0]-b[0]));
