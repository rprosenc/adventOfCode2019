...
const intcode = function(programm) {
	...
	const add = () => {
		...
		let term1 = posMode(i,1) ? memory[p1] : p1;
		let term2 = posMode(i,2) ? memory[p2] : p2;
		memory[p3] = term1 + term2;
		log('add: written '+ memory[p3] +' to memory['+ p3 +']');
	}
	const mul = () => {
		p1 = ldi();
		p2 = ldi();
		...
		let factor1 = posMode(i,1) ? memory[p1] : p1;
		let factor2 = posMode(i,2) ? memory[p2] : p2;
		memory[p3] = factor1 * factor2;
		log('mul: written '+ memory[p3] +' to memory['+ p3 +']');
	}
	const inp = () => {
		p1 = ldi();
		memory[p1] = input();
	...
	const out = () => {
		p1 = ldi();
		let value = posMode(i,1) ? memory[p1] : p1;
		output(value);
	}
...
const posMode = (v,p) => {
	let m = 1 * ( (''+v).split('').reverse().splice(2)[p-1] );
	log('posMode('+v+', '+p+') -> '+ m);
	return !m;
}

const input = () => 1; // input for now constant '1'
